...
export function InteractiveAI() {
  ...
  const [history, setHistory] = useState<Array<{ sender: "user" | "assistant"; text: string }>>([
    {
      sender: "assistant",
      text: "Ask any question about Aryan's work or projects. This portfolio chatbot is powered by Google Gemini when the server has a Gemini API key configured.",
    },
  ]);
  ...
  return (
    <div className="rounded-[2rem] border border-foreground/10 bg-card/90 p-6 shadow-[0_28px_90px_rgba(17,17,17,0.12)]">
      <div className="mb-6">
        ...
        <h3 className="mt-4 text-3xl font-semibold tracking-tight">Gemini Resume Chat + Project Q&A</h3>
        <p className="mt-3 text-sm leading-6 text-muted-foreground">
          Ask any question and receive a generated answer from Google Gemini using Aryan's portfolio context.
        </p>
      </div>
      ...
      <div className="mb-4 max-h-[320px] space-y-4 overflow-y-auto rounded-[1.75rem] border border-foreground/10 bg-background/80 p-5 text-sm text-muted-foreground shadow-inner">
        {history.map((message, idx) => (
          <div key={idx} className="space-y-2">
            <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{message.sender === "user" ? "You" : "Gemini"}</p>
